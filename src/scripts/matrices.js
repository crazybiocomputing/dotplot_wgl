...
g.matMan.list = [];
g.matMan.currentType = null;

g.matMan.updateDOM = function() {
    var type;
    try {
        type = (
            g.DOM.opt1.options[g.DOM.opt1.selectedIndex].dataset.type === "dna" &&
            g.DOM.opt2.options[g.DOM.opt2.selectedIndex].dataset.type === "dna"
        ) ? "nucleic" : "proteic";
    } catch (err) {
        return;
    }
    if (g.matMan.currentType !== type) {
        g.matMan.list.forEach(function(mat) {
...
(function() {
    ...
    ["DNA Full", "DNA Mat", "DNA Simple", "Identity"].forEach(init, {type: "nucleic", size: 17});
    if (g.DOMLoaded) {
        g.matMan.updateDOM();
        g.DOM.opt1.addEventListener("change", g.matMan.updateDOM, false);
        g.DOM.opt2.addEventListener("change", g.matMan.updateDOM, false);
    } else {
